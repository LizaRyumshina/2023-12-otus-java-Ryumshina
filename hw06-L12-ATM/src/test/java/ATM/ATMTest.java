...
public class ATMTest {
    Banknote banknoteType_5000;
    Banknote banknoteType_1000;
    Banknote banknoteType_100;
    ATM atm;
    @BeforeEach
    void createATMBeforeEach() {

        banknoteType_5000 = new Banknote(5000);
        banknoteType_1000 = new Banknote(1000);
        banknoteType_100 = new Banknote(100);
        atm = new ATM(4);
    }
    @Test
    ...
    void putBanknoteATMTest() {
        // нет кассет
        assertThrows(Throwable.class, () -> {atm.depositCash(banknoteType_5000, 1);});

        // добавили кассету для наминала 1000 и 10 купюр
        CashCassette cassette = new CashCassette(banknoteType_1000,2000);
        atm.addCashCassette(cassette);
        int addedBanknote1 = 10;
        atm.depositCash(banknoteType_1000, addedBanknote1);
        assertThat(atm.getBalanceCash()).isEqualTo(addedBanknote1*banknoteType_1000.getDenomination());

        // нет подходящей кассеты для номинала 5000
        assertThrows(Throwable.class, () -> {atm.depositCash(banknoteType_5000, 1);});

        // добавили кассету для наминала 5000 и 5 купюр
        CashCassette cassette2 = new CashCassette(banknoteType_5000,2000);
        atm.addCashCassette(cassette2);
        int addedBanknote2 = 5;
        atm.depositCash(banknoteType_5000, addedBanknote2);
        assertThat(atm.getBalanceCash()).isEqualTo(addedBanknote1*banknoteType_1000.getDenomination()+addedBanknote2*banknoteType_5000.getDenomination());
    }
    @Test
    ...
    void getBanknoteATMTest() {
        // нет кассет
        assertThrows(Throwable.class, () -> {atm.withdrawCash(123);});

        // добавили кассету для наминала 100 и 100 купюр
        CashCassette cassette = new CashCassette(banknoteType_100,2000);
        atm.addCashCassette(cassette);
        int addedBanknote1 = 100;
        atm.depositCash(banknoteType_100, addedBanknote1);
        int balance= addedBanknote1*banknoteType_100.getDenomination();

        //пытаемся получить сумму меньше минимального наминала
        assertThrows(Throwable.class, () -> {atm.withdrawCash(50);});
        ...
        atm.depositCash(banknoteType_5000, addedBanknote2);
        balance+= banknoteType_5000.getDenomination()*addedBanknote2; //balance = 509500

        // получаем 80 купюр наминалом 5000 и 11 наминалом 100
        List<StackBanknotes> stackBanknotes= atm.withdrawCash(401100);
        //System.out.println(stackBanknotes);
        balance-=401100;
        assertThat(atm.getBalanceCash()).isEqualTo(balance);
    }
    @Test
    ...
    void getBalanceCashATMTest() {
        ...
        CashCassette cassette2 = new CashCassette(banknoteType_100,2000);
        int addedBanknote2 = 2000;
        cassette2.addBanknotes(addedBanknote2);
        atm.addCashCassette(cassette2);
        assertThat(atm.getBalanceCash()).isEqualTo(addedBanknote1*banknoteType_1000.getDenomination() + addedBanknote2*banknoteType_100.getDenomination());

    }
    @Test
    void addRemoveCassetteATMTest() {
        CashCassette cassette1 = new CashCassette(banknoteType_1000, 100);
        atm.addCashCassette(cassette1);
        assertThrows(Throwable.class, () -> {atm.addCashCassette(cassette1);}); //already installed
        ...
        atm.addCashCassette(new CashCassette(banknoteType_1000, 10));
        atm.addCashCassette(new CashCassette(banknoteType_1000, 20));
        atm.addCashCassette(new CashCassette(banknoteType_1000, 30));
